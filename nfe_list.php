...
include_once __DIR__.'/class/nfe.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('fiscal', 'nfe', 'write');
	$permissiontodelete = $user->hasRight('fiscal', 'nfe', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("fiscal")) {
	accessforbidden('Module fiscal not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	$objectclass = 'NFe';
	$objectlabel = 'NFe';
	$uploaddir = $conf->fiscal->dir_output;
...
$title = $langs->trans("NFes");
//$help_url = "EN:Module_NFe|FR:Module_NFe_FR|ES:Módulo_NFe";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/fiscal/nfe_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-fiscal page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for a horizontal scroll in the table instead of page
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/fiscal/nfe_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendNFeRef";
$modelmail = "nfe";
$objecttmp = new NFe($db);
...
if ($search_all) {
	...
	print '<!-- Search done on configured NF-e quicksearch fields: '.$setupstring.' -->'."\n";
