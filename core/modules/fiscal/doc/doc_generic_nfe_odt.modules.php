...
/**
 *	\file       htdocs/core/modules/fiscal/doc/doc_generic_nfe_odt.modules.php
 *	\ingroup    fiscal
 *	\brief      File of class to build ODT documents for nfes
 */

dol_include_once('/fiscal/core/modules/fiscal/modules_nfe.php');
 ...
 */
class doc_generic_nfe_odt extends ModelePDFNFe
{
	/**
	 * Issuer
	 * @var Societe
	...
	public function __construct($db)
	{
		...
		$this->scandir = 'FISCAL_NFE_ADDON_PDF_ODT_PATH'; // Name of constant that is used to save list of directories to scan
	...
	public function info($langs)
	{
		...
		$odtPath = trim(getDolGlobalString('FISCAL_NFE_ADDON_PDF_ODT_PATH'));
		...
		$texte .= '<input type="hidden" name="param1" value="FISCAL_NFE_ADDON_PDF_ODT_PATH">';
		...
		if ($nbofiles) {
			...
			foreach ($listoffiles as $file) {
				$texte .= '- '.$file['name'].' <a href="'.DOL_URL_ROOT.'/document.php?modulepart=doctemplates&file=fiscal_nfe/'.urlencode(basename($file['name'])).'">'.img_picto('', 'listlight').'</a>';
				$texte .= ' &nbsp; <a class="reposition" href="'.$_SERVER["PHP_SELF"].'?modulepart=doctemplates&keyforuploaddir=FISCAL_NFE_ADDON_PDF_ODT_PATH&action=deletefile&token='.newToken().'&file='.urlencode(basename($file['name'])).'">'.img_picto('', 'delete').'</a>';
				$texte .= '<br>';
			}
			$texte .= '</div>';
		}
		...
		if (!getDolGlobalString('MAIN_NO_MULTIDIR_FOR_ODT')) {
			...
			$texte .= '<br>';
			$texte .= '<input type="hidden" name="value1" value="FISCAL_NFE_ADDON_PDF_ODT_PATH">';
		}
		...
		$texte .= '<input type="hidden" value="FISCAL_NFE_ADDON_PDF_ODT_PATH" name="keyforuploaddir">';
	 ...
	 *
	 *  @param	NFe	$object				Source object to generate document from
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->fiscal->dir_output) {
			// If $object is id instead of object
			if (!is_object($object)) {
				$id = $object;
				$object = new NFe($this->db);
			...
			$dir = $conf->fiscal->multidir_output[isset($object->entity) ? $object->entity : 1];
			...
			if (file_exists($dir)) {
				...
				dol_mkdir($conf->fiscal->dir_temp);
				if (!is_writable($conf->fiscal->dir_temp)) {
					$this->error = $langs->transnoentities("ErrorFailedToWriteInTempDirectory", $conf->fiscal->dir_temp);
				...
				// Line of free text
				$newfreetext = '';
				$paramfreetext = 'FISCAL_NFE_FREE_TEXT';
				...
				try {
					$odfHandler = new Odf(
						$srctemplatepath,
						array(
							'PATH_TO_TMP'	  => $conf->fiscal->dir_temp,
